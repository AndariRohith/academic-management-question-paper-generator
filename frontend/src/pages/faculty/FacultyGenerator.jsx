...
const FacultyGenerator = () => {
    ...
    const fetchSubjects = async (regId, semester) => {
        try {
            ...
            // Note: the generator API (port 5005) returns subjects.
            // We need to check if these subjects are assigned to the user.
            // However, the generator API might not return 'faculty_assign'.
            // If it does, great. If not, we might need to cross-ref with the Subject Service (port 5003).
            // For now, I'll assume the generator API returns standard subject objects.
            // If filtering fails, I will trust the user (or fetched data) for now.
            // But let's try to fetch full subject details from Subject Service to confirm assignment.

            // To imply security, we will execute a client-side filter using a separate fetch to Subject Service
            const subjectsUrl = `http://127.0.0.1:5003/get_subjects?regulation=${regId}&semester=${semester}`;
            const subjRes = await fetch(subjectsUrl);
            const subjData = await subjRes.json();

            const facultySubjects = subjData.filter(s =>
                s.faculty_assign &&
                s.faculty_assign.toLowerCase() === user.username.toLowerCase()
            );

            // Map back to the format used by Generator if needed, but usually they are consistent.
            // The generator dropdown uses: id, code, name

            const filtered = facultySubjects.map(s => ({
                id: s.subject_id,
                code: s.subject_code,
                name: s.subject_name
            }));
    ...
    const handleGeneratePapers = async () => {
        ...
        setGenerating(true);
        try {
            const selectedSubject = subjects.find(s => s.id == formData.subject); // Loose equality for potential type mismatch
    ...
    return (
        <div className="generator-page">
            <div className="page-header">
                ...
                <p>Generate papers for your assigned subjects</p>
            </div>
            ...
            <div className="content-card">
                ...
                <div className="form-grid">
                    ...
                    <div className="form-group">
                        ...
                        <select name="semester" value={formData.semester} onChange={handleInputChange} disabled={!formData.regulation}>
                            <option value="">-- Select --</option>
                            {semesters.map((sem) => (
                    ...
                    <div className="form-group">
                        <label>Subject (Assigned Only)</label>
                        <select name="subject" value={formData.subject} onChange={handleInputChange} disabled={!formData.semester}>
                            <option value="">-- Select --</option>
                            {subjects.map((subj) => (
            ...
            {showUploadAndResults && (
                <div className="content-card">
                    ...
                    <div className="form-actions">
                        <button className="btn btn-success btn-lg" onClick={handleGeneratePapers} disabled={generating || !parsedId}>
                ...
                <div className="content-card">
                    ...
                    {generatedPapers.length === 0 ? (
                        <div className="empty-state">
                            ...
                            <small>Upload a question bank and click generate</small>
                        </div>
                    ) : (
            ...
            {toast.visible && (
                <div className="toast-container">
                    <div className={`toast ${toast.type}`}>
                        ...
                        <div className="toast-content"><p className="toast-message">{toast.message}</p></div>
                    </div>
                </div>
            )}
        </div>
...
export default FacultyGenerator;
